...
1. **저장 실패 가드(즉시·완료)**: `persist()`가 용량 초과 등으로 조용히 던져 데이터가 사라지던 문제를 try/catch로 감싸 사용자에게 알리도록 수정함(`db.saveError`). 로고/직인 base64가 ~5MB `localStorage` 한도를 잠식하는 게 원인.
2. 근본 대책: (a) 업로드 이미지 리사이즈·압축, 또는 (b) 저장소를 **IndexedDB로 이전**(용량 큼)해 견적·이미지 누적에 대비. 한도 임박 시 경고.
3. 백업 스키마 버전·마이그레이션: `_v` 태그로 옛 백업 변환, 가져오기 검증 강화. "마지막 백업: N일 전" 환기(선택).
...
4. 상태 전이 규칙: 승인/취소 견적의 수정 정책(잠금 또는 경고).
