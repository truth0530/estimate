...
| 영역 | 결정 | 이유 |
|------|------|------|
| 프론트엔드 | SvelteKit + Svelte 5 | 작은 업무 앱에 충분하고 Supabase SSR 인증 문서가 있다. |
| 백엔드/BaaS | Supabase | PostgreSQL, Auth, Storage, RLS를 한 번에 쓸 수 있다. |
...
PDF/Excel 라이브러리는 출력 화면 구현 시점에 추가한다.

```bash
npm i exceljs pdfmake
```

### Phase 1. 데이터와 인증

1. Supabase 프로젝트 생성
2. 테이블, 인덱스, FK, RLS, Storage 정책 작성
3. SvelteKit Supabase SSR 설정
4. `/login`과 보호 라우트 구현
5. RLS 접근 테스트 작성
...
2. 수량/가격 파서 구현
3. 신뢰도 낮은 행 표시
4. 실제 품목 데이터로 오탐/누락 조정

### Phase 6. 확장 검토

- 전자세금계산서: 서버 API, 공급업체, 인증서, 비용 검토 후 별도 계획
- 매출 대시보드: quotes 집계 기반으로 추가
- iOS: 모바일 웹 사용성이 검증된 뒤 Capacitor 래핑

---

## 10. 테스트 기준

- RLS: 다른 사용자 데이터 조회/수정/삭제 불가
- 견적번호: 동시에 2개 생성해도 중복 없음
- 원자성: `save_quote` 중간에 실패해도 고아 라인 없이 전부 롤백
- 무결성: 클라이언트가 조작한 `amount`/합계를 보내도 DB 생성 컬럼·RPC 계산값으로 저장
- 품목 단가 변경 후에도 과거 견적 라인 금액 유지
- 품목 소프트 삭제 시 자동완성엔 안 뜨고 과거 견적은 유지, 하드 삭제 시 `item_id`만 null·라인 보존
- VAT 계산이 `floor` 기준으로 일관됨
- 면세 설정 시 세액 0원 처리
- PDF 한글, 로고, 직인 표시
- Excel 병합, 테두리, 금액 서식 표시
- 모바일 화면에서 라인 추가/삭제/수정이 불편하지 않음
- 로그아웃 상태에서 보호 라우트 접근 불가

---

## 11. 운영

- Supabase 무료 플랜은 프로젝트 일시정지와 백업 부재가 가장 큰 리스크다.
- 하루 1회 keepalive를 둘 수 있지만, 무료 정책 변경 가능성이 있으므로 배포 직전 최신 요금제를 다시 확인한다.
- 주 1회 `supabase db dump` 또는 `pg_dump` 백업을 만든다.
- service role key는 클라이언트 번들에 절대 넣지 않는다.
- `.env`는 로컬과 배포 환경 변수로만 관리한다.
